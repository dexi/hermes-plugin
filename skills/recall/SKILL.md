...
4. If the user has saved a base for it (`dexi_bases`), `dexi_query_base` runs their own filter and sort — "show me my reading list", "what's due this week" — and returns each note's `key:: value` properties plus, when the view defines them, the base's computed `formulas`, the note's `group` with a top-level `groups` list (label, count, per-group summaries) and column `summaries` over the whole result. Prefer it over re-deriving the filter yourself, and read totals from `summaries` rather than adding them up.
